...
=== "I Need The Main Entry Point"
    ...
    - `kayak.open_text_retriever(...)` for one high-level plain-text workflow
...
| --- | --- | --- |
| ingest plain text and search with one object | `kayak.open_text_retriever(...)` | [Usage Patterns](usage-patterns.md) |
...
This constructor expects text strings plus an explicit encoder. It does not own
OCR, document parsing, extraction, or answer generation.
...
High-level workflow object for plain-text ingest plus search.
